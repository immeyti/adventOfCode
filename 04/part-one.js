...
fs.readFile('./inputs.txt', 'utf8', (err, originalData) => { // Reading the inputs
    ...
    pairsIdRang.forEach(pair => {
        ...
        if ((firsElv.start <= secondElv.start && firsElv.end >= secondElv.end) || 
            (firsElv.start >= secondElv.start && firsElv.end <= secondElv.end)) {
            duplicated += 1;
            console.log(extractedPair);
        }
    });

    console.log(duplicated, pairsIdRang.length);
});